...
class AA:
    ...
    def analyse(self, K: int = 3, p: int = 6, n_iter: int = 1000, early_stopping: bool = False, model_type = "all", lr: float = 0.1, mute: bool = False, with_synthetic_data: bool = False, with_hot_start: bool = False):
        ...
        if self._has_data and not with_synthetic_data:
            if model_type == "all" or model_type == "CAA":
                ...
                print("The AA_type \"{0}\" specified, does not match any of the possible AA_types.".format(model_type))
    ...
    def plot(self, 
        ...
        if not model_type in ["CAA", "OAA", "RBOAA", "TSAA"]:
            ...
            if result_number < 0 or not result_number < len(self._results[model_type]):
                ...
                result._plot(plot_type,attributes,archetype_number,types,weighted,subject_indexes,save_figure,filename)
    ...
    # def create_dataframe(self, model_type: = "CAA", result_number = 0):
